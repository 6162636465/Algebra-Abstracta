...
public:
    ...
    int zztoint(ZZ);
    string obtenerstring(string);
    string permutacion1(string);
    string permutacion2(string);
    string generarclavek(string, string , int);
    string correizquierda(string, int);
    string sumatodoslosk(string );
    string clavedef(int,string);
    string permutacion(string );
...
string RC4::correizquierda(string mensaje, int x) {
	string res = mensaje;
	char al;
	for (int i = 0; i < x; i++) {
		bitset<28> m(res);
...
string RC4::permutacion1(string mensaje) {
	string retorna;
...
int RC4::moduloint(int a, int b) {
	if (a >= 0)
		return a - (a / b) * b;
	else
		return a - ((a / b) - 1) * b;
}
string RC4::permutacion(string mensaje) {
	string retorna;
	int tabla[] = { 56, 48, 40, 32, 24, 16, 8, 0, 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35, 62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 60, 52, 44, 36, 28, 20, 12, 4, 27, 19, 11, 3 };
	int size = sizeof(tabla) / sizeof(tabla[0]);
	for (int i = 0; i < size; i++)
...
string RC4::clavedef(int num, string base){
...
string RC4::sumatodoslosk(string base){
...
string RC4::generarclavek(string c, string d, int x) {
...
string RC4::permutacion2(string mensaje) {
	string retorna;
...
string RC4::obtenerstring(string mensaje) {
	string algo, men;
